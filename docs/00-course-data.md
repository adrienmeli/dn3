
 
# Bases de données



##  Vocabulaire 


\begin{longtable}{>{\raggedright\arraybackslash}p{8cm}>{\raggedright\arraybackslash}p{8cm}}
\toprule
Français & English\\
\midrule
\cellcolor{gray!6}{flou} & \cellcolor{gray!6}{blurry}\\
rugueux & rough\\
\cellcolor{gray!6}{doux (au toucher)} & \cellcolor{gray!6}{smooth}\\
brouillard & fog\\
\cellcolor{gray!6}{brume} & \cellcolor{gray!6}{mist}\\
\addlinespace
la partie inférieure & the bottom part\\
\cellcolor{gray!6}{la partie supérieure} & \cellcolor{gray!6}{the upper part}\\
aborder (un sujet) & to tackle\\
\cellcolor{gray!6}{à haute criminalité} & \cellcolor{gray!6}{crime-ridden}\\
a ridge & une crête\\
\addlinespace
\cellcolor{gray!6}{atroce, ou criard} & \cellcolor{gray!6}{lurid}\\
attendre de qn que... & to expect sb to\\
\cellcolor{gray!6}{au premier plan} & \cellcolor{gray!6}{in the foreground}\\
biaisé, partial & biassed\\
\cellcolor{gray!6}{ce qui est produit, ce qui sort} & \cellcolor{gray!6}{the output}\\
\addlinespace
chaos & mayhem\\
\cellcolor{gray!6}{comestible} & \cellcolor{gray!6}{edible}\\
destinataire, récipiendaire & a recipient\\
\cellcolor{gray!6}{diffuser} & \cellcolor{gray!6}{to broadcast}\\
disposer, agencer & to lay out\\
\addlinespace
\cellcolor{gray!6}{durer} & \cellcolor{gray!6}{to last}\\
échapper à & to elude\\
\cellcolor{gray!6}{esquisser} & \cellcolor{gray!6}{to sketch}\\
être condamné & to be doomed\\
\cellcolor{gray!6}{être susceptible de} & \cellcolor{gray!6}{to be likely to}\\
\addlinespace
évident & obvious\\
\cellcolor{gray!6}{exact, précis} & \cellcolor{gray!6}{accurate}\\
faire 1m. de hauteur & to be 1m. high\\
\cellcolor{gray!6}{faire 1m. de large} & \cellcolor{gray!6}{to be 1m. wide}\\
faire 1m. de profondeur & to be 1m. deep\\
\addlinespace
\cellcolor{gray!6}{fournir (p...)} & \cellcolor{gray!6}{to provide sb with}\\
fournir (s...) & to supply sb with\\
\cellcolor{gray!6}{il semble que} & \cellcolor{gray!6}{it looks as if}\\
une tache, une zone & a patch\\
\cellcolor{gray!6}{incliner} & \cellcolor{gray!6}{to tilt}\\
\addlinespace
intenter un procès & to sue\\
\cellcolor{gray!6}{interdire} & \cellcolor{gray!6}{to ban}\\
la majorité écrasante & the overwhelming majority\\
\cellcolor{gray!6}{la poitrine} & \cellcolor{gray!6}{the chest}\\
la respiration, l’haleine & the breath\\
\addlinespace
\cellcolor{gray!6}{le confinement} & \cellcolor{gray!6}{the lockdown}\\
le coucher du soleil & the sunset\\
\cellcolor{gray!6}{le sein maternel} & \cellcolor{gray!6}{the bosom}\\
menacer & to threaten\\
\cellcolor{gray!6}{pendant que} & \cellcolor{gray!6}{while}\\
\addlinespace
posséder qqch & to own stg\\
\cellcolor{gray!6}{postuler à qqch} & \cellcolor{gray!6}{to apply for stg}\\
promouvoir (a...) & to advertise\\
\cellcolor{gray!6}{promouvoir (p...)} & \cellcolor{gray!6}{to promote}\\
provenir de & to stem from\\
\addlinespace
\cellcolor{gray!6}{résoudre} & \cellcolor{gray!6}{to work out}\\
s’accroupir & to crouch\\
\cellcolor{gray!6}{selon, d’après} & \cellcolor{gray!6}{according to}\\
se noyer & to drown\\
\cellcolor{gray!6}{(se) terminer} & \cellcolor{gray!6}{to be over}\\
\addlinespace
sonder & to probe\\
\cellcolor{gray!6}{un but, un objectif (a...)} & \cellcolor{gray!6}{an aim}\\
un but, un objectif (p...) & a purpose\\
\cellcolor{gray!6}{un contour} & \cellcolor{gray!6}{an outline}\\
un écart (d...) & a discrepancy\\
\addlinespace
\cellcolor{gray!6}{un écart (g...)} & \cellcolor{gray!6}{a gap}\\
une épave de bateau & a shipwreck\\
\cellcolor{gray!6}{une légende d’image} & \cellcolor{gray!6}{a caption}\\
une mission, une tâche & an assignment\\
\cellcolor{gray!6}{une pièce de 10 ¢} & \cellcolor{gray!6}{a dime}\\
\addlinespace
une pièce de 25 ¢ & a quarter\\
\cellcolor{gray!6}{une pièce de 5 ¢} & \cellcolor{gray!6}{a nickel}\\
une tâche de couleur & a patch\\
\cellcolor{gray!6}{une tempête} & \cellcolor{gray!6}{a storm}\\
une toile & a canvas\\
\addlinespace
\cellcolor{gray!6}{une vague} & \cellcolor{gray!6}{a wave}\\
une vue d’ensemble & an overview\\
\cellcolor{gray!6}{un hommage} & \cellcolor{gray!6}{a tribute}\\
un indice & a clue\\
\cellcolor{gray!6}{un ordinateur de bureau} & \cellcolor{gray!6}{a desktop}\\
\addlinespace
un ordinateur portable & a laptop\\
\cellcolor{gray!6}{un rédacteur en chef} & \cellcolor{gray!6}{an editor}\\
un sondage & a poll\\
\cellcolor{gray!6}{un stage} & \cellcolor{gray!6}{an internship}\\
un traité (livre) & a treatise\\
\addlinespace
\cellcolor{gray!6}{vendre la mèche} & \cellcolor{gray!6}{to spill the beans}\\
élargir & to widen\\
\cellcolor{gray!6}{approfondir} & \cellcolor{gray!6}{to deepen}\\
rehausser & to heighten\\
\cellcolor{gray!6}{allonger} & \cellcolor{gray!6}{to lengthen}\\
\addlinespace
largeur & width\\
\cellcolor{gray!6}{longueur} & \cellcolor{gray!6}{length}\\
profondeur & depth\\
\cellcolor{gray!6}{hauteur} & \cellcolor{gray!6}{height}\\
riche, abondant, aisé & wealthy\\
\addlinespace
\cellcolor{gray!6}{concret} & \cellcolor{gray!6}{hands-on}\\
un devoir & a duty\\
\cellcolor{gray!6}{la sagesse} & \cellcolor{gray!6}{wisdom}\\
être excité à l’idée de & to be hung up on...\\
\cellcolor{gray!6}{l’état-providence} & \cellcolor{gray!6}{the welfare state}\\
\addlinespace
le noyau & the core\\
\cellcolor{gray!6}{une revendication} & \cellcolor{gray!6}{a claim}\\
recouper, inclure & to cut across\\
\cellcolor{gray!6}{un festin} & \cellcolor{gray!6}{a feast}\\
mêler à & to embroil\\
\addlinespace
\cellcolor{gray!6}{supporter, accepter} & \cellcolor{gray!6}{to bear}\\
cinglant & searing\\
\cellcolor{gray!6}{une hypothèse} & \cellcolor{gray!6}{an assumption}\\
sans fard, simple et clair & bald\\
\cellcolor{gray!6}{une tentative} & \cellcolor{gray!6}{an endeavour}\\
\addlinespace
lutter & to struggle\\
\cellcolor{gray!6}{vaincre} & \cellcolor{gray!6}{to overcome}\\
une limite, une frontière & a border\\
\cellcolor{gray!6}{superficiel, peu profond} & \cellcolor{gray!6}{shallow}\\
une énigme & a riddle\\
\addlinespace
\cellcolor{gray!6}{lugubre (b...)} & \cellcolor{gray!6}{bleak}\\
lugubre (d...) & dismal\\
\cellcolor{gray!6}{lugubre (g...)} & \cellcolor{gray!6}{gloomy}\\
une polémique & a row (rime avec “now“)\\
\cellcolor{gray!6}{avant} & \cellcolor{gray!6}{prior to}\\
\addlinespace
à l’aube de & on the eve of\\
\cellcolor{gray!6}{une répétition (théâtrale)} & \cellcolor{gray!6}{a rehearsal}\\
une foule & a crowd\\
\cellcolor{gray!6}{un manque} & \cellcolor{gray!6}{a lack}\\
une enquête officielle & an enquiry\\
\addlinespace
\cellcolor{gray!6}{cassant, fragile} & \cellcolor{gray!6}{brittle}\\
somptueux & lavish\\
\cellcolor{gray!6}{un incendie} & \cellcolor{gray!6}{a blaze}\\
tenir compte de qqch (t...) & to take stg into account\\
\cellcolor{gray!6}{tenir compte de qqch (a...)} & \cellcolor{gray!6}{to allow for stg}\\
\addlinespace
authentique & genuine\\
\cellcolor{gray!6}{soulever un problème} & \cellcolor{gray!6}{to raise an issue}\\
une usine & a factory\\
\cellcolor{gray!6}{une locomotive à vapeur} & \cellcolor{gray!6}{a steam engine}\\
un chemin, un sentier & a path\\
\addlinespace
\cellcolor{gray!6}{un produit, un bien de consommation} & \cellcolor{gray!6}{a good}\\
dilapider & to squander\\
\cellcolor{gray!6}{éviter} & \cellcolor{gray!6}{to avoid}\\
dentelle & lace\\
\cellcolor{gray!6}{clocher} & \cellcolor{gray!6}{steeple}\\
\addlinespace
confiance (en soi) & confidence\\
\cellcolor{gray!6}{deviner} & \cellcolor{gray!6}{to guess}\\
“tenir debout“, “se tenir“ & to have legs\\
\cellcolor{gray!6}{épineux, délicat} & \cellcolor{gray!6}{tricky}\\
franchir une ligne & to cross a line\\
\addlinespace
\cellcolor{gray!6}{arrogant} & \cellcolor{gray!6}{overbearing}\\
réaction, retour & feedback\\
\cellcolor{gray!6}{une position} & \cellcolor{gray!6}{a stance}\\
une personne facile à convaincre, crédule & a pushover\\
\cellcolor{gray!6}{têtu} & \cellcolor{gray!6}{stubborn}\\
\addlinespace
amadouer & to schmooze\\
\cellcolor{gray!6}{cahoteux, accidenté} & \cellcolor{gray!6}{bumpy}\\
lisse, sans heurts & smooth\\
\cellcolor{gray!6}{to tackle} & \cellcolor{gray!6}{to set about dealing with}\\
un accord, une approbation & an approval\\
\addlinespace
\cellcolor{gray!6}{papeterie} & \cellcolor{gray!6}{stationery}\\
finir par faire qqch & to wind up doing stg\\
\cellcolor{gray!6}{véhiculer, transmettre} & \cellcolor{gray!6}{to convey}\\
rappeler qqch à qn & to remind sb of stg\\
\cellcolor{gray!6}{convaincre qn de dépenser plus} & \cellcolor{gray!6}{to upsell}\\
\addlinespace
attendre qqch de qn & to expect sb to do stg\\
\cellcolor{gray!6}{un modèle} & \cellcolor{gray!6}{a template}\\
une affiche & a billboard\\
\cellcolor{gray!6}{audacieux} & \cellcolor{gray!6}{bold}\\
finir par... & to end up doing...\\
\addlinespace
\cellcolor{gray!6}{très long} & \cellcolor{gray!6}{lengthy}\\
épais & thick\\
\cellcolor{gray!6}{couper} & \cellcolor{gray!6}{to trim}\\
plier & to fold\\
\cellcolor{gray!6}{peser} & \cellcolor{gray!6}{to weigh}\\
\addlinespace
d’après, selon & according to\\
\cellcolor{gray!6}{lisse, doux} & \cellcolor{gray!6}{smooth}\\
réunir & to gather\\
\cellcolor{gray!6}{le dos d’un livre} & \cellcolor{gray!6}{the spine}\\
une échéance & a deadline\\
\addlinespace
\cellcolor{gray!6}{engager, embaucher} & \cellcolor{gray!6}{to hire}\\
dépenser & to spend\\
\cellcolor{gray!6}{par exemple} & \cellcolor{gray!6}{for instance}\\
once & une fois que\\
\cellcolor{gray!6}{célèbre} & \cellcolor{gray!6}{famous}\\
\addlinespace
rendre, donner & to hand out\\
\cellcolor{gray!6}{tôt} & \cellcolor{gray!6}{early}\\
une réunion & a meeting\\
\cellcolor{gray!6}{fournir} & \cellcolor{gray!6}{to provide}\\
une séance photo & a photo shoot\\
\addlinespace
\cellcolor{gray!6}{la maquette, la disposition} & \cellcolor{gray!6}{the layout}\\
une approbation, un accord & an approval\\
\cellcolor{gray!6}{une publicité, une annonce} & \cellcolor{gray!6}{an ad}\\
planifier & to schedule\\
\cellcolor{gray!6}{a sample} & \cellcolor{gray!6}{un échantillon}\\
\addlinespace
un récit & a tale\\
\cellcolor{gray!6}{d’actualité} & \cellcolor{gray!6}{topical}\\
finalement & eventually\\
\cellcolor{gray!6}{être conscient de, savoir} & \cellcolor{gray!6}{to be aware of}\\
dépeindre & to depict\\
\addlinespace
\cellcolor{gray!6}{to overstep} & \cellcolor{gray!6}{outrepasser}\\
accomplir & to achieve\\
\cellcolor{gray!6}{grand plaisir, joie} & \cellcolor{gray!6}{delight}\\
stupéfait, abasourdi & astounded\\
\cellcolor{gray!6}{agité, qui ne prend jamais de répit} & \cellcolor{gray!6}{restless}\\
\addlinespace
to depict & représenter\\
\cellcolor{gray!6}{bien que} & \cellcolor{gray!6}{although}\\
pleurer, déplorer & to mourn\\
\cellcolor{gray!6}{essayer de} & \cellcolor{gray!6}{to attempt to}\\
sauver, préserver & to salvage\\
\addlinespace
\cellcolor{gray!6}{un huissier} & \cellcolor{gray!6}{a bailiff}\\
comme si & though\\
\cellcolor{gray!6}{avare} & \cellcolor{gray!6}{miserly}\\
un parasite & a hanger-on\\
\cellcolor{gray!6}{se déclencher} & \cellcolor{gray!6}{to break out}\\
\addlinespace
sauvage & wild\\
\cellcolor{gray!6}{une preuve} & \cellcolor{gray!6}{a proof}\\
le commerce, un métier & trade\\
\cellcolor{gray!6}{un péché} & \cellcolor{gray!6}{a sin}\\
en dépit de & despite\\
\addlinespace
\cellcolor{gray!6}{emprunter à} & \cellcolor{gray!6}{to borrow from}\\
le public & the audience\\
\cellcolor{gray!6}{une échelle (de grandeur)} & \cellcolor{gray!6}{a scale}\\
homologue, équivalent, pair & fellow\\
\cellcolor{gray!6}{un rappel} & \cellcolor{gray!6}{a reminder}\\
\addlinespace
flou & blurred\\
\cellcolor{gray!6}{santé} & \cellcolor{gray!6}{health}\\
rassembler & to gather\\
\cellcolor{gray!6}{éteindre} & \cellcolor{gray!6}{to shut down}\\
un conseil & a tip\\
\addlinespace
\cellcolor{gray!6}{au lieu de} & \cellcolor{gray!6}{instead of}\\
un fossé, un écart & a gap\\
\cellcolor{gray!6}{permettre à qn de faire qqch} & \cellcolor{gray!6}{to enable sb to do stg}\\
une zone tampon & a buffer\\
\cellcolor{gray!6}{d’avant, ancien} & \cellcolor{gray!6}{former}\\
\addlinespace
temps libre & downtime\\
\cellcolor{gray!6}{désirer ardemment} & \cellcolor{gray!6}{to crave}\\
environ (après un nombre) & odd\\
\cellcolor{gray!6}{pendant} & \cellcolor{gray!6}{while}\\
une pause, une interruption & a break\\
\addlinespace
\cellcolor{gray!6}{échanger} & \cellcolor{gray!6}{to swap}\\
un confinement & a lockdown\\
\cellcolor{gray!6}{à distance} & \cellcolor{gray!6}{remote}\\
le surmenage & burnout\\
\cellcolor{gray!6}{le train-train quotidien, les corvées} & \cellcolor{gray!6}{drudgery}\\
\addlinespace
un créneau & a slot\\
\cellcolor{gray!6}{une humeur} & \cellcolor{gray!6}{a mood}\\
au moins & at least\\
\cellcolor{gray!6}{aller chercher, atteindre} & \cellcolor{gray!6}{to fetch}\\
engranger, récolter & to bag\\
\addlinespace
\cellcolor{gray!6}{un rejeton, une ramification} & \cellcolor{gray!6}{an offshoot}\\
une sérigraphie & a screenprint\\
\cellcolor{gray!6}{défigurer} & \cellcolor{gray!6}{to deface}\\
un bien, \cellcolor{gray!6}{un actif, un atout} & \cellcolor{gray!6}{an asset}\\
\cellcolor{gray!6}{déchiqueter, réduire en lambeaux} & \cellcolor{gray!6}{to shred}\\
\addlinespace
une vente aux enchères & an auction\\
\cellcolor{gray!6}{un dollar (fam.)} & \cellcolor{gray!6}{a buck}\\
un gros titre & a headline\\
\cellcolor{gray!6}{être témoin de qqch} & \cellcolor{gray!6}{to witness stg}\\
un point de bascule & a tipping point\\
\addlinespace
\cellcolor{gray!6}{un décor, un arrière-plan} & \cellcolor{gray!6}{a backdrop}\\
caracoler & to prance\\
\cellcolor{gray!6}{contempler, regarder fixement} & \cellcolor{gray!6}{to gaze at}\\
être bloqué & to be stuck\\
\cellcolor{gray!6}{un objet, un jeton, une marque} & \cellcolor{gray!6}{a token}\\
\addlinespace
la propriété, la possession & ownership\\
\cellcolor{gray!6}{cupide} & \cellcolor{gray!6}{greedy}\\
une arnaque & a scam\\
un actif, un atout & an asset\\
une devise, une monnaie & a currency\\
\addlinespace
\cellcolor{gray!6}{un engouement, une mode} & \cellcolor{gray!6}{a craze}\\
improbable & unlikely\\
\cellcolor{gray!6}{une enchère, une offre d’achat} & \cellcolor{gray!6}{a bid}\\
suivre la dernière tendance & to jump on the bandwagon\\
\cellcolor{gray!6}{une escroquerie} & \cellcolor{gray!6}{a con}\\
\addlinespace
posséder & to own\\
\cellcolor{gray!6}{au-delà de} & \cellcolor{gray!6}{beyond}\\
détruire, faire disparaître & to wipe out\\
\cellcolor{gray!6}{pourrir, se décomposer} & \cellcolor{gray!6}{to decay}\\
étendre, étirer & to stretch\\
\addlinespace
\cellcolor{gray!6}{l’artisanat, le talent artistique} & \cellcolor{gray!6}{craft}\\
une tache & a blotch\\
\cellcolor{gray!6}{donner la parole} & \cellcolor{gray!6}{to give the floor}\\
gronder, passer un savon & to tell off\\
\cellcolor{gray!6}{une pièce exposée} & \cellcolor{gray!6}{an exhibit}\\
\addlinespace
un événement & an event\\
\cellcolor{gray!6}{diffuser, retransmettre} & \cellcolor{gray!6}{to broadcast*}\\
une cascade, un coup de pub & a stunt\\
\cellcolor{gray!6}{révéler} & \cellcolor{gray!6}{to disclose}\\
transformer & to turn into\\
\addlinespace
\cellcolor{gray!6}{puisque} & \cellcolor{gray!6}{since}\\
ajouter & to add\\
\cellcolor{gray!6}{derrière} & \cellcolor{gray!6}{behind}\\
échanger, commercer & to trade\\
\cellcolor{gray!6}{un service public} & \cellcolor{gray!6}{a utility}\\
\addlinespace
chercher & to watch for\\
\cellcolor{gray!6}{une flambée, une poussée} & \cellcolor{gray!6}{a surge}\\
une monnaie, une devise & a currency\\
\cellcolor{gray!6}{un actif, un bien, un atout} & \cellcolor{gray!6}{an asset}\\
une enchère & an auction\\
\addlinespace
\cellcolor{gray!6}{un droit, une prétention} & \cellcolor{gray!6}{a claim}\\
assaillir & to beset\\
\cellcolor{gray!6}{une étagère} & \cellcolor{gray!6}{a rack}\\
une cabane & a shed\\
\cellcolor{gray!6}{surgir brusquement} & \cellcolor{gray!6}{to spring up}\\
\addlinespace
se connecter & to hook up\\
\cellcolor{gray!6}{un entrepôt} & \cellcolor{gray!6}{a warehouse}\\
une rangée & a row\\
\cellcolor{gray!6}{un logiciel} & \cellcolor{gray!6}{a software}\\
la richesse & wealth\\
\addlinespace
\cellcolor{gray!6}{un terrain, un domaine} & \cellcolor{gray!6}{a realm}\\
un impact, un dégât & a toll\\
\cellcolor{gray!6}{manipuler, forcer} & \cellcolor{gray!6}{to tamper}\\
un registre & a ledger\\
\cellcolor{gray!6}{des honoraires, des frais} & \cellcolor{gray!6}{fees}\\
\addlinespace
un cadre, un environnement & a framework\\
\cellcolor{gray!6}{décoller} & \cellcolor{gray!6}{to take off}\\
détaché & unmoored\\
\cellcolor{gray!6}{changer, transférer} & \cellcolor{gray!6}{to shift}\\
verser & to pour\\
\addlinespace
\cellcolor{gray!6}{une installation} & \cellcolor{gray!6}{a setup}\\
un engouement & a craze\\
\cellcolor{gray!6}{peu économique, qui gaspille} & \cellcolor{gray!6}{wasteful}\\
sensibiliser & to raise awareness\\
\cellcolor{gray!6}{un bandit de grand chemin} & \cellcolor{gray!6}{a highway robber}\\
\addlinespace
les biens, les possessions & the effects\\
\cellcolor{gray!6}{tandis que} & \cellcolor{gray!6}{while}\\
ne pas être conscient de & to be oblivious to\\
\cellcolor{gray!6}{un moyens de} & \cellcolor{gray!6}{a means to}\\
sympathique & congenial\\
\addlinespace
\cellcolor{gray!6}{le premier plan} & \cellcolor{gray!6}{the foreground}\\
prévenir de & to warn against\\
\cellcolor{gray!6}{une corde} & \cellcolor{gray!6}{a rope}\\
une bagarre & a brawl\\
\cellcolor{gray!6}{le goût, le parfum} & \cellcolor{gray!6}{the flavour}\\
\addlinespace
cuire au four & to bake\\
\cellcolor{gray!6}{cautionner} & \cellcolor{gray!6}{to endorse}\\
parmi & among(st)\\
\cellcolor{gray!6}{avaler} & \cellcolor{gray!6}{to swallow}\\
embrasser & to snog\\
\addlinespace
\cellcolor{gray!6}{s’évanouir} & \cellcolor{gray!6}{to faint}\\
un accessoire & a prop\\
\cellcolor{gray!6}{une récompense} & \cellcolor{gray!6}{an award}\\
précédent & previous\\
\cellcolor{gray!6}{un couvercle} & \cellcolor{gray!6}{a lid}\\
\addlinespace
du jour au lendemain & overnight\\
\cellcolor{gray!6}{une échelle} & \cellcolor{gray!6}{a scale}\\
évaluer & to rate\\
\cellcolor{gray!6}{14 sur 20} & \cellcolor{gray!6}{14 out of 20}\\
s’attendre à qqch & to expect stg\\
\addlinespace
\cellcolor{gray!6}{des résultats mitigés} & \cellcolor{gray!6}{mixed results}\\
l’opinion majoritaire & the majority opinion\\
\cellcolor{gray!6}{tranché, clair} & \cellcolor{gray!6}{clear-cut}\\
appartenir à & to belong to\\
\cellcolor{gray!6}{la plupart} & \cellcolor{gray!6}{most}\\
\addlinespace
un tiers & a third\\
\cellcolor{gray!6}{un veau} & \cellcolor{gray!6}{a calf}\\
un mollet & a calf\\
\cellcolor{gray!6}{un mémoire étudiant} & \cellcolor{gray!6}{a research paper}\\
une entreprise & a company\\
\addlinespace
\cellcolor{gray!6}{une infime majorité} & \cellcolor{gray!6}{a tiny majority}\\
jusqu'à maintenant & so far\\
\cellcolor{gray!6}{aller à l'étranger} & \cellcolor{gray!6}{to go abroad}\\
être indécis & to sit on a fence\\
\cellcolor{gray!6}{un but (p...)} & \cellcolor{gray!6}{a purpose}\\
\addlinespace
un résumé & a summary\\
\cellcolor{gray!6}{se concentrer sur} & \cellcolor{gray!6}{to focus on}\\
viser à, avoir pour but de & to aim to\\
\cellcolor{gray!6}{dès le départ} & \cellcolor{gray!6}{right off the bat}\\
dans quelle mesure & to what extent\\
\addlinespace
\cellcolor{gray!6}{mettre à nu} & \cellcolor{gray!6}{to lay bare}\\
les résultats & the findings\\
\cellcolor{gray!6}{un moyen de} & \cellcolor{gray!6}{a means to}\\
un résultat (o...) & an outcome\\
\cellcolor{gray!6}{une tendance} & \cellcolor{gray!6}{a trend}\\
\addlinespace
aborder un problème & to address an issue\\
\cellcolor{gray!6}{exécution, mise en œuvre} & \cellcolor{gray!6}{implementation}\\
un échantillon & a sample\\
\cellcolor{gray!6}{accomplir, réaliser} & \cellcolor{gray!6}{to carry out}\\
inattendu & unexpected\\
\addlinespace
\cellcolor{gray!6}{attirer} & \cellcolor{gray!6}{to pull in}\\
déclarer & to state\\
\cellcolor{gray!6}{un plan de mesures} & \cellcolor{gray!6}{a scheme}\\
faire l'éloge de & to praise\\
\cellcolor{gray!6}{se moquer de} & \cellcolor{gray!6}{to poke fun at}\\
\addlinespace
inclure & to comprise\\
\cellcolor{gray!6}{en apparence} & \cellcolor{gray!6}{seemingly}\\
accueillir, héberger & to host\\
\cellcolor{gray!6}{se mettre en route, chercher à} & \cellcolor{gray!6}{to set out}\\
une recette & a recipe\\
\addlinespace
\cellcolor{gray!6}{à l'intérieur de} & \cellcolor{gray!6}{within}\\
attacher, fermer & to fasten\\
\cellcolor{gray!6}{un vendeur au détail} & \cellcolor{gray!6}{a retailer}\\
pouvoir s'acheter qqch & to afford something\\
\cellcolor{gray!6}{abandonner} & \cellcolor{gray!6}{to give up}\\
\addlinespace
améliorer, faire des progrès & to improve\\
\cellcolor{gray!6}{rendre un hommage} & \cellcolor{gray!6}{to pay a tribute}\\
un vif désir & a compulsion\\
\cellcolor{gray!6}{fierté, orgueil} & \cellcolor{gray!6}{pride}\\
du mobilier, des meubles & furniture\\
\addlinespace
\cellcolor{gray!6}{rester, demeurer} & \cellcolor{gray!6}{to remain}\\
s'enrôler & to enlist\\
\cellcolor{gray!6}{fort probable} & \cellcolor{gray!6}{likely}\\
le ravitaillement & the supply\\
\cellcolor{gray!6}{déménager} & \cellcolor{gray!6}{to move}\\
\addlinespace
demeurer & to remain\\
\cellcolor{gray!6}{décerner} & \cellcolor{gray!6}{to award}\\
champ, portée, marge de manœuvre & scope\\
\cellcolor{gray!6}{bien aimer qqch} & \cellcolor{gray!6}{to be fond of stg}\\
agencement, disposition, maquette & a layout\\
\addlinespace
\cellcolor{gray!6}{un scrupule} & \cellcolor{gray!6}{a qualm}\\
qui s'ensuit, final & eventual\\
\cellcolor{gray!6}{un équipage} & \cellcolor{gray!6}{a crew}\\
probable, susceptible & likely\\
\cellcolor{gray!6}{s'excuser de qqch} & \cellcolor{gray!6}{to apologize}\\
\addlinespace
agile & nimble\\
\cellcolor{gray!6}{avertir} & \cellcolor{gray!6}{to warn}\\
diriger qqch & to head stg\\
\cellcolor{gray!6}{rendement, productivité} & \cellcolor{gray!6}{output}\\
manipuler qqch & to handle stg\\
\addlinespace
\cellcolor{gray!6}{confier qqch à qn} & \cellcolor{gray!6}{to entrust sb with stg}\\
une esquisse & a sketch\\
\cellcolor{gray!6}{une forme} & \cellcolor{gray!6}{a shape}\\
continuer, poursuivre & to carry on\\
\cellcolor{gray!6}{vivant, plein d'entrain} & \cellcolor{gray!6}{lively}\\
\addlinespace
reconnaître, admettre & to acknowledge\\
\bottomrule
\end{longtable}



##  Définitions 


\begin{longtable}{>{\raggedright\arraybackslash}p{8cm}>{\raggedright\arraybackslash}p{8cm}}
\toprule
Expression & Definition\\
\midrule
\cellcolor{gray!6}{a career} & \cellcolor{gray!6}{a profession for which one trains and which is undertaken as a permanent calling}\\
a commune & a group of families or single people who live and work together sharing possessions and responsibilities\\
\cellcolor{gray!6}{an output} & \cellcolor{gray!6}{something produced}\\
a skill & a learned power of doing something competently\\
\cellcolor{gray!6}{a treatise} & \cellcolor{gray!6}{a formal piece of writing that considers and examines a particular subject}\\
\addlinespace
lurid & causing horror or revulsion\\
\cellcolor{gray!6}{to be doomed} & \cellcolor{gray!6}{to be condemned, to  be certain to experience something terrible}\\
to figure out & to discover\\
\cellcolor{gray!6}{wit} & \cellcolor{gray!6}{astuteness of perception or apt humour}\\
a one-off & limited to a single time, occasion, or instance\\
\addlinespace
\cellcolor{gray!6}{to epitomise} & \cellcolor{gray!6}{to embody}\\
to commission & to appoint or assign to a task or function\\
\cellcolor{gray!6}{a hook} & \cellcolor{gray!6}{something intended to attract and ensnare}\\
to repurpose & to give a new use to\\
\cellcolor{gray!6}{a tendril} & \cellcolor{gray!6}{a stem modified into a slender organ serving to attach a climbing plant to its support}\\
\addlinespace
tuition fees & the price of or payment for instruction\\
\cellcolor{gray!6}{to plaster} & \cellcolor{gray!6}{to affix to or place on especially conspicuously or in quantity}\\
to dispel & to cause to disappear\\
\cellcolor{gray!6}{to elude} & \cellcolor{gray!6}{to escape the perception, understanding, or grasp of}\\
a recipient & one that receives\\
\addlinespace
\cellcolor{gray!6}{an aim} & \cellcolor{gray!6}{a clearly directed intent or purpose}\\
a purpose & something set up as an object or end to be attained\\
\cellcolor{gray!6}{to stem from} & \cellcolor{gray!6}{to be caused by, to come from}\\
an overview & a general survey, a summary\\
\cellcolor{gray!6}{an assignment} & \cellcolor{gray!6}{a specified task or amount of work given by authority}\\
\addlinespace
to work out & to solve (something, such as a problem) by a process of reasoning or calculation\\
\cellcolor{gray!6}{to sketch} & \cellcolor{gray!6}{to make a rough draft, or outline of}\\
accurate & free from error especially as the result of care\\
\cellcolor{gray!6}{to sue} & \cellcolor{gray!6}{to seek justice or right from (a person) by legal process}\\
to drop & to cease to be of concern\\
\addlinespace
\cellcolor{gray!6}{to sell like hot cakes} & \cellcolor{gray!6}{to sell at a rapid rate}\\
to crouch & to lower the body stance especially by bending the legs\\
\cellcolor{gray!6}{edible} & \cellcolor{gray!6}{something that is suitable or safe to eat}\\
to threaten & to express an intention to inflict evil, injury, or damage\\
\cellcolor{gray!6}{according to} & \cellcolor{gray!6}{in conformity with, as stated by}\\
\addlinespace
to come through & to be expressed or communicated\\
\cellcolor{gray!6}{a clue} & \cellcolor{gray!6}{a piece of evidence that leads one toward the solution of a problem}\\
prominence & something widely and popularly known\\
\cellcolor{gray!6}{a backdrop} & \cellcolor{gray!6}{a painted cloth hung across the rear of a stage}\\
to rail & to revile or scold in harsh, insolent, or abusive language\\
\addlinespace
\cellcolor{gray!6}{wealthy} & \cellcolor{gray!6}{very affluent, characterized by abundance}\\
a parable & a usually short fictitious story that illustrates a moral attitude or a religious principle\\
\cellcolor{gray!6}{a duty} & \cellcolor{gray!6}{obligatory tasks, conduct, service, or functions that arise from one's position}\\
wisdom & accumulated philosophical or scientific learning\\
\cellcolor{gray!6}{to be hung up (on stg)} & \cellcolor{gray!6}{having great or excessive interest in or preoccupation with someone or something}\\
\addlinespace
a claim & a demand for something due or believed to be due\\
\cellcolor{gray!6}{to cut across} & \cellcolor{gray!6}{to include within the scope of effect or significance}\\
to embroil & to involve in conflict or difficulties\\
\cellcolor{gray!6}{pervasive} & \cellcolor{gray!6}{existing in or spreading through every part of something}\\
squalor & a state of being marked by filthiness and degradation from neglect or poverty\\
\addlinespace
\cellcolor{gray!6}{bald} & \cellcolor{gray!6}{basic and with no unnecessary words or detail}\\
to endeavour & to attempt\\
\cellcolor{gray!6}{to outlaw} & \cellcolor{gray!6}{to make illegal}\\
hideous & offensive to the senses and especially to sight\\
\cellcolor{gray!6}{to rail against sb} & \cellcolor{gray!6}{to revile or scold in harsh, insolent, or abusive language}\\
\addlinespace
a row & a noisy argument or fight\\
\cellcolor{gray!6}{a slum} & \cellcolor{gray!6}{a very poor and crowded area, especially of a city}\\
to imbue & to fill something or someone with a quality or feeling\\
\cellcolor{gray!6}{an outbreak} & \cellcolor{gray!6}{a sudden beginning}\\
a foreman & the supervisor or leader of a group\\
\addlinespace
\cellcolor{gray!6}{skewed} & \cellcolor{gray!6}{not accurate or exact}\\
scrutiny & careful, detailed examination or inspection\\
\cellcolor{gray!6}{to account for something} & \cellcolor{gray!6}{to explain or to be the reason for}\\
refurbishment & the process of improving a building or store by cleaning and decorating it, adding new furniture, equipment, etc.\\
\cellcolor{gray!6}{to pillory} & \cellcolor{gray!6}{to severely criticize someone, especially in a public way}\\
\addlinespace
to urge sb to do stg & \\
\cellcolor{gray!6}{buttoned-up} & \cellcolor{gray!6}{coldly reserved or standoffish}\\
to overshadow & to cause someone or something to seem less important or noticeable\\
\cellcolor{gray!6}{an outlook} & \cellcolor{gray!6}{a view from a particular place, a prospect in the future}\\
to taint & to spoil something or give it an unpleasant quality\\
\addlinespace
\cellcolor{gray!6}{to plague} & \cellcolor{gray!6}{to cause worry, pain, or difficulty to someone or something over a period of time}\\
to claim & to say that something is true or is a fact, although you cannot prove it and other people might not believe it\\
\cellcolor{gray!6}{to shelter} & \cellcolor{gray!6}{to protect yourself, or another person or thing, from bad weather, danger, or attack}\\
to pour & to make a substance flow from a container\\
\cellcolor{gray!6}{a dweller} & \cellcolor{gray!6}{a person who lives in a city, town, cave, etc.}\\
\addlinespace
a row & a line of things, people, animals, etc. arranged next to each other\\
\cellcolor{gray!6}{to unsettle} & \cellcolor{gray!6}{to make someone feel anxious or slightly worried}\\
handicraft & a skilled activity in which something is made in a traditional way with the hands rather than being produced by machines in a factory\\
\cellcolor{gray!6}{a seat} & \cellcolor{gray!6}{a place that acts as a base or centre for an important activity}\\
falsehood & a lie or a statement that is not correct\\
\addlinespace
\cellcolor{gray!6}{a bed} & \cellcolor{gray!6}{the bottom of something, or something that serves as a base}\\
to squander & to waste money or supplies, or to waste opportunities by not using them to your advantage\\
\cellcolor{gray!6}{tracery} & \cellcolor{gray!6}{architectural ornamental work with branching lines}\\
filigree & delicate jewellery made from twisted wire\\
\cellcolor{gray!6}{lacework} & \cellcolor{gray!6}{objects or patterns consisting of or resembling lace}\\
\addlinespace
picturesque & attractive in appearance, especially in an old-fashioned way\\
\cellcolor{gray!6}{to beat to the punch} & \cellcolor{gray!6}{to do or achieve something before someone else is able to}\\
beforehand & in anticipation, in advance\\
\cellcolor{gray!6}{a pain point} & \cellcolor{gray!6}{a persistent or recurring problem (as with a product or service) that frequently inconveniences or annoys customers}\\
to poke holes in stg & to find mistakes or problems in a plan or in what someone has said\\
\addlinespace
\cellcolor{gray!6}{a takeaway} & \cellcolor{gray!6}{a conclusion to be made based on presented facts or information}\\
to tweak stg & to make usually small adjustments in or to  stg\\
\cellcolor{gray!6}{momentum} & \cellcolor{gray!6}{strength or force gained by motion or by a series of events}\\
a journey & an act or instance of traveling from one place to another\\
\cellcolor{gray!6}{to come easy} & \cellcolor{gray!6}{to be easy to achieve or accomplish}\\
\addlinespace
to buzz & to make a low continuous humming sound like that of a bee\\
\cellcolor{gray!6}{a step} & \cellcolor{gray!6}{an action, proceeding, or measure often occurring as one in a series}\\
to walk through & to guide (someone, such as a novice) through an unfamiliar or complex procedure step-by-step\\
\cellcolor{gray!6}{to pitch} & \cellcolor{gray!6}{to present or advertise}\\
stellar & outstanding, exceptional\\
\addlinespace
\cellcolor{gray!6}{to push over the edge} & \cellcolor{gray!6}{o cause one to commit to doing something they had been considering or close to doing for a long time.}\\
a buy-in & acceptance of and willingness to actively support and participate in something\\
\cellcolor{gray!6}{a daydream} & \cellcolor{gray!6}{a pleasant visionary usually wishful creation of the imagination}\\
amazeballs & extremely surprising or good:\\
\cellcolor{gray!6}{the layout} & \cellcolor{gray!6}{the way that something is arranged}\\
\addlinespace
a head start & an advantage that someone has over other people in something such as a competition or race\\
\cellcolor{gray!6}{hefty} & \cellcolor{gray!6}{quite heavy}\\
a brand & a public image, reputation, or identity conceived of as something to be marketed or promoted\\
\cellcolor{gray!6}{main} & \cellcolor{gray!6}{chief, principal}\\
to drive & to give shape or impulse to\\
\addlinespace
\cellcolor{gray!6}{stunning} & \cellcolor{gray!6}{strikingly impressive especially in beauty or excellence}\\
a mock-up & a full-sized structural model built to scale chiefly for study, testing, or display\\
\cellcolor{gray!6}{to upsell} & \cellcolor{gray!6}{to try to convince (a customer) to purchase something additional or at a higher cost}\\
to flesh out & to provide more information about something, to make something more complete by adding details\\
\cellcolor{gray!6}{to draw out} & \cellcolor{gray!6}{to remove, to extract}\\
\addlinespace
to cleanse & to rid of impurities by or as if by washing\\
\cellcolor{gray!6}{signage} & \cellcolor{gray!6}{all the signs that advertise a product or company}\\
closure & an often comforting or satisfying sense of finality\\
\cellcolor{gray!6}{hip} & \cellcolor{gray!6}{very fashionable, trendy}\\
premium & a high value or a value in excess of that normally or usually expected\\
\addlinespace
\cellcolor{gray!6}{to wrap up} & \cellcolor{gray!6}{to summarize, to bring to a usually successful conclusion}\\
Analogous colours & colours adjacent to one another on the colour wheel (e.g. violet, red and orange).\\
\cellcolor{gray!6}{Complementary colours} & \cellcolor{gray!6}{opposites on the colour wheel. This relationship will produce visual tension and “shock.”}\\
Triadic colours & three colours evenly spaced on the colour wheel. One colours dominates, the second supports, and the third accents.\\
\cellcolor{gray!6}{Gradient} & \cellcolor{gray!6}{a gradual change from one colour to another. (For example, blue transitioning gradually to green).}\\
\addlinespace
Opacity & synonymous with non-transparency. The more transparent an image, the lower its opacity.\\
\cellcolor{gray!6}{kerning} & \cellcolor{gray!6}{the adjustment of space between pairs of letters in the same word.}\\
kerning & adds or subtracts space between letters to create more visually appealing and readable text.\\
\cellcolor{gray!6}{leading} & \cellcolor{gray!6}{also known as line-height, the space between two lines of text.}\\
tracking & the adjustment of space for groups of letters and entire blocks of text.\\
\addlinespace
\cellcolor{gray!6}{tracking} & \cellcolor{gray!6}{affects every character in the selected text and is used to change its overall appearance.}\\
Paper color & Most paper is white. However, there are hundreds of different shades of white. Some whites are very warm, almost yellow. Others are cool, almost blue or gray.\\
\cellcolor{gray!6}{Paper coating} & \cellcolor{gray!6}{How smooth the surface of the paper feels. The degree of smoothness is created during the paper-making process.}\\
Paper weight & Paper is graded according to its weight — which is usually expressed in terms of how much 500 sheets of the paper in its standard size weighs.\\
\cellcolor{gray!6}{Strength} & \cellcolor{gray!6}{how well the paper holds up under stress. Paper bags and envelopes need a high degree of strength.}\\
\addlinespace
Thickness & Thick papers don’t have to weigh a lot. Some books are printed on very thick but lightweight paper, which makes the book look like it has more pages.\\
\cellcolor{gray!6}{Brightness} & \cellcolor{gray!6}{how light reflects off  the paper. Some papers contain fluorescents so they appear brighter. This makes the paper sparkle more, but can affect the color of printed images.}\\
Opacity & refers to how much the text or images printed on the other side of the page show through.\\
\cellcolor{gray!6}{Three-ring binding} & \cellcolor{gray!6}{uses three punched holes in the paper and a three-ring binder.}\\
Plastic comb binding & uses a plastic insert with teeth that fit into rectangular holes in the paper.\\
\addlinespace
\cellcolor{gray!6}{Spiral binding} & \cellcolor{gray!6}{uses a metal or plastic spiral that coils through many small holes on the side of the paper.}\\
Wir-O binding & similar to spiral binding, but instead of a single spiral, two wire teeth fit into rectangular holes in the paper.\\
\cellcolor{gray!6}{Velo binding} & \cellcolor{gray!6}{uses two plastic strips on either side of the document. The strips are held together with plastic pins and bound together with heat.}\\
Fastback binding & uses a cloth or paper strip wrapped around the spine of the pages and then glued in position.\\
\cellcolor{gray!6}{Saddle-stitch binding} & \cellcolor{gray!6}{uses two or more staples inserted right at the fold of the paper.}\\
\addlinespace
Side-stitch binding & combines all the signatures and the cover and stitches them together with staples on the outside of the book cover.\\
\cellcolor{gray!6}{Perfect binding} & \cellcolor{gray!6}{gathers all the signatures together. The spine is then ground to create a fl at edge, and a paper cover is glued around the spine.}\\
Lay-flat binding & gathers all the signatures together and grinds the spine as in perfect binding. Th e cover is then glued to the book at each side of the spine, which allows the pages of the book to lay flat when opened.\\
\cellcolor{gray!6}{a scheme} & \cellcolor{gray!6}{an officially organized plan or system}\\
to poke fun at sb & to make someone seem stupid by making jokes about them or laughing unkindly\\
\addlinespace
\cellcolor{gray!6}{to praise} & \cellcolor{gray!6}{to express admiration or approval of the achievements or characteristics of a person or thing}\\
to comprise stg & to have things or people as parts or members, to consist of stg\\
\cellcolor{gray!6}{to set out} & \cellcolor{gray!6}{to start an activity with a particular aim}\\
seemingly & appearing to be something, especially when this is not true\\
\cellcolor{gray!6}{fat} & \cellcolor{gray!6}{the substance under the skin of humans and animals that stores energy and keeps them warm}\\
\addlinespace
within & inside or not further than an area or period of time\\
\cellcolor{gray!6}{to host} & \cellcolor{gray!6}{to provide the space and other things necessary for a special event}\\
to fasten & to (cause something to) become firmly fixed together, or in position, or closed\\
\cellcolor{gray!6}{a hodgepodge} & \cellcolor{gray!6}{a heterogeneous mixture}\\
a retailer & someone who sells in small quantities directly to the consumer\\
\addlinespace
\cellcolor{gray!6}{pride} & \cellcolor{gray!6}{a reasonable or justifiable self-respect}\\
to afford & to be able to pay\\
\cellcolor{gray!6}{to give up} & \cellcolor{gray!6}{to stop trying to do something before you have finished}\\
a parlour & a room for customers usually of firms providing particular services\\
\cellcolor{gray!6}{a compulsion} & \cellcolor{gray!6}{a strong feeling that you must do something}\\
\addlinespace
to improve & to get or make better\\
\cellcolor{gray!6}{a tribute} & \cellcolor{gray!6}{something that is done to honor somebody}\\
a powerhouse & a country, organization, or person with a lot of influence\\
\cellcolor{gray!6}{an obituary} & \cellcolor{gray!6}{a notice of a person's death usually with a short biographical account}\\
a craft & an occupation or trade requiring manual dexterity or artistic skill\\
\addlinespace
\cellcolor{gray!6}{a monotype} & \cellcolor{gray!6}{an impression on paper of a design painted usually with the finger or a brush on a surface (such as glass)}\\
scholarly & containing a serious detailed study of a subject\\
\cellcolor{gray!6}{endpaper} & \cellcolor{gray!6}{a once-folded sheet of paper having one leaf pasted flat against the inside of the front or back cover of a book and the other pasted at the base to the first or last page}\\
cautionary & giving a warning\\
\cellcolor{gray!6}{to commission stg} & \cellcolor{gray!6}{to request sb to create a piece of art}\\
\addlinespace
a typescript & a typed copy of a piece of writing such as a book\\
\cellcolor{gray!6}{to work out} & \cellcolor{gray!6}{to happen or develop in a particular way}\\
\bottomrule
\end{longtable}



##  Traductions 


\begin{longtable}{>{\raggedright\arraybackslash}p{8cm}>{\raggedright\arraybackslash}p{8cm}}
\toprule
Français & English\\
\midrule
\cellcolor{gray!6}{Sur quoi Milton Glaser a-t-il dessiné le logo ?} & \cellcolor{gray!6}{What did he draw the logo on?}\\
Il l'a dessiné sur une enveloppe. & He drew it on an envelope.\\
\cellcolor{gray!6}{L'enveloppe sur laquelle il a dessiné le logo est exposée au MoMA.} & \cellcolor{gray!6}{The envelope he drew the logo on is exhibited / displayed in MoMA.}\\
L'homme qui a créé le logo est mort en juillet. & The man who designed the logo died in July.\\
\cellcolor{gray!6}{Pour comprendre le logo, il faut résoudre une petite énigme.} & \cellcolor{gray!6}{You need to crack / solve a little riddle to understand the logo.}\\
\addlinespace
Si Milton Glaser n'avait pas inventé le logo de New-York, il ne serait pas mondialement connu. & If Milton Glaser hadn't designed the NYC logo, he would not be world-wide famous.\\
\cellcolor{gray!6}{Le monde serait-il meilleur si les publicités ne mentaient pas ?} & \cellcolor{gray!6}{Would the world be a better place if adverts didn't lie?}\\
Milton Glaser conseille aux jeunes artistes de beaucoup travailler. & Milton Glaser advises young artists to work a lot.\\
\cellcolor{gray!6}{Il aurait pu être un peu plus optimiste.} & \cellcolor{gray!6}{He could have been a bit more optimistic.}\\
Il se peut qu'il ait été pessimiste... & He may have been pessimistic.\\
\addlinespace
\cellcolor{gray!6}{Mais j'aurais quand même dû l'écouter !} & \cellcolor{gray!6}{Still, I should have listened to him!}\\
Il est temps que tu ailles au lit. & It is time you went to bed.\\
\cellcolor{gray!6}{Je préfèrerais que tu lui parles tout de suite.} & \cellcolor{gray!6}{I'd rather you talked to him right now.}\\
Je regrette que tu ne lui aies pas parlé plus tôt. & I wish you had spoken to him earlier.\\
\cellcolor{gray!6}{La toile fait 1,22m. de large.} & \cellcolor{gray!6}{The canvas is 1.22m wide.}\\
\addlinespace
Elle est exposée au musée des Beaux Arts de Boston. & It is displayed in the museum of Fine Arts in Boston\\
\cellcolor{gray!6}{Il se peut qu'elle ait été peinte au XIXè siècle.} & \cellcolor{gray!6}{It may have been painted in the 19th century.}\\
Je regrette de ne pas avoir vu ce tableau & I wish I had seen this painting.\\
\cellcolor{gray!6}{Le cours de la semaine dernière a duré 3 heures, ce qui était beaucoup trop long.} & \cellcolor{gray!6}{Last week's lesson lasted 3 hours, which was way too long.}\\
Ce que j'ai trouvé un peu exagéré, ce sont les poissons et les mouettes au premier plan. & What I found a bit exaggerated were the seagulls and fish in the foreground.\\
\addlinespace
\cellcolor{gray!6}{Ce tableau, que Ruskin a si bien décrit, est en fait très politique.} & \cellcolor{gray!6}{This painting, which Ruskin described so well, is actually very political.}\\
Si Ruskin n'avait pas écrit sur Turner, celui-ci ne serait pas célèbre aujourd'hui & Turner wouldn't be famous today had Ruskin not written about him,\\
\cellcolor{gray!6}{Pugin a peut-être  poussé Ruskin à écrire} & \cellcolor{gray!6}{Pugin may have led Ruskin to take to writing.}\\
On peut dire de lui qu'il a été un véritable visionnaire & He can be said to have been a true visionary.\\
\cellcolor{gray!6}{On devrait tenir compte de la pensée victorienne dans nos opinions} & \cellcolor{gray!6}{Victorian thought should be allowed for in our opinions.}\\
\addlinespace
Peu de gens connaissent Pugin, à la différence de Turner dont les oeuvres sont mondialement connues & Few people know Pugin, unlike Turner, whose works are famous world-wide.\\
\cellcolor{gray!6}{Pourtant il a conçu l'un des bâtiments anglais les plus connus.} & \cellcolor{gray!6}{He designed one of the most famous English landmarks, though.}\\
L'image du haut est plus détaillée que celle du bas & The top picture features more details than the bottom one.\\
\cellcolor{gray!6}{Les maisons georgiennes semblent lugubres tandis qu'on a envie de rentrer dans cette auberge néo-gothique} & \cellcolor{gray!6}{Georgian houses look bleak whereas you really want to step inside this neo-Gothic inn.}\\
The crowd began to trickle back home. & La foule se mit à rentrer au compte-gouttes\\
\addlinespace
\cellcolor{gray!6}{They were rubbing their eyes, which even in the gloom could be seen to be red-rimmed} & \cellcolor{gray!6}{Ils se frottaient les yeux, dont on pouvait discerner les bords rougis dans l'obscurité}\\
La foule au premier plan est à peine visible & The crowd in the foreground can hardly be seen.\\
\cellcolor{gray!6}{Cette huile-sur-toile est exposée au musée de Philadelphie} & \cellcolor{gray!6}{This oil-on-canvas is displayed at the Philadelphia Museum of Art.}\\
Turner a passé des heures à retravailler le tableau avant de le montrer. & Turner spent hours reworking the painting before exhibiting it.\\
\cellcolor{gray!6}{On pensait que ces esquisses montraient l'incendie du Parlement.} & \cellcolor{gray!6}{}\\
\addlinespace
Avec qui travailles-tu sur le logo du cours ? & Who are you working on the logo with?\\
\cellcolor{gray!6}{On dit de nos nouveaux clients qu'ils n'aiment pas gaspiller de l'argent} & \cellcolor{gray!6}{Our new customers are said not to like squandering money.}\\
J'ai vraiment hâte de présenter notre projet & I'm really hung up on pitching our project.\\
\cellcolor{gray!6}{Je regrette de ne pas avoir tenu compte des problèmes soulevés par le client} & \cellcolor{gray!6}{I wish I had taken into account the issues raised by the customer.}\\
Vantablack est utilisé par Anish Kapoor depuis 2014. & VB has been used by Kapoor since 2014.\\
\addlinespace
\cellcolor{gray!6}{Depuis mon enfance, j'ai toujours aimé la couleur.} & \cellcolor{gray!6}{Ever since I was a child, I've always loved colours.}\\
La forme importe plus que la couleur & Shape matters more than colour\\
\cellcolor{gray!6}{Voilà trois mois que je réfléchis à ce projet.} & \cellcolor{gray!6}{I’ve been thinking about this project for three months.}\\
Les résultats fournissent aux concepteurs graphiques des conseils supplémentaires. & The findings provide designers with additional advice.\\
\cellcolor{gray!6}{L’étude cherche à déterminer dans quelle mesure les pratiques actuelles sont durables et écologiques.} & \cellcolor{gray!6}{The survey investigates how green and sustainable the current procedures are.}\\
\addlinespace
Les professionnels que nous avons interrogés disent tous que leurs clients attachent beaucoup d’importance au développement durable. & The professionals we have interviewed all say their customers value sustainability.\\
\cellcolor{gray!6}{Le problème que cet article aborde m'intéresse vivement} & \cellcolor{gray!6}{I'm highly interested in the issue this article addresses.}\\
Si j'avais su, j'aurais pris le temps de développer mes compétences artistiques & If I had known, I would have taken the time to hone my artistic skills.\\
\cellcolor{gray!6}{De nombreux graphistes se sont engagés à promouvoir le développement durable} & \cellcolor{gray!6}{Many graphic designers are committed to promoting sustainability.}\\
Sur quoi travailles-tu en ce moment? & What are you working on these days?\\
\addlinespace
\cellcolor{gray!6}{Je finis un projet sur lequel je travaille depuis deux mois.} & \cellcolor{gray!6}{I am finishing a project I have been working on for two months.}\\
Quelles sont tes échéances ? & What are your deadlines?\\
\cellcolor{gray!6}{Je dois finir d'ici la fin de la semaine.} & \cellcolor{gray!6}{I must finish by the end of the week.}\\
J'ai déjà contacté plus de dix entreprises depuis septembre. & I've alreay contacted more than 10 companies since September.\\
\cellcolor{gray!6}{J'ai contacté plus de dix entreprises en septembre.} & \cellcolor{gray!6}{I contacted more than 10 companies in September.}\\
\addlinespace
J'aimerais attirer votre attention sur la raffinement des illustrations. & I would like to draw your attention to the exquisiteness of the illustrations.\\
\cellcolor{gray!6}{La mise en page vise à mettre le texte en valeur.} & \cellcolor{gray!6}{The layout aims at enhance the text.}\\
Même le dos du livre présente des qualités esthétiques dignes d'être mentionnées. & Even the spine of the book features aesthetic qualities that are worth mentioning.\\
\cellcolor{gray!6}{J'ai choisi cet article parce que la même chose m'est arrivée.} & \cellcolor{gray!6}{I chose this article because the same thing (has) happened to me.}\\
Voilà une aventure dont j'aurais bien fait l'expérience. & This is an adventure I would have loved to experience.\\
\addlinespace
\cellcolor{gray!6}{Si mon projet se concrétise, j'irai travailler à l'étranger.} & \cellcolor{gray!6}{If my project goes ahead, I'll go and work abroad.}\\
\bottomrule
\end{longtable}



##  Prononciation 


\begin{longtable}{lll}
\toprule
Word & Stress & Received Pronunciation\\
\midrule
\cellcolor{gray!6}{about} & \cellcolor{gray!6}{/01/} & \cellcolor{gray!6}{/\textipa{@."baUt}/}\\
accurate & /100/ & /\textipa{"\ae k.j@r.@t}/\\
\cellcolor{gray!6}{although} & \cellcolor{gray!6}{/01/} & \cellcolor{gray!6}{/\textipa{O:l."D@U}/}\\
bosom & /10/ & /\textipa{"bUz.@m}/\\
\cellcolor{gray!6}{breath} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{breT}/}\\
\addlinespace
budgetary & /1000/ & /\textipa{"b2dZ.It.@r.i}/\\
\cellcolor{gray!6}{canvas} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"k\ae n.v@s}/}\\
commune & /01/ & /\textipa{k@."mju:n}/\\
\cellcolor{gray!6}{contribute} & \cellcolor{gray!6}{/010/} & \cellcolor{gray!6}{/\textipa{k@n."trIb.ju:t}/}\\
country & /10/ & /\textipa{"k2ntr.i}/\\
\addlinespace
\cellcolor{gray!6}{crouch} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{kraUtS}/}\\
domain & /01/ & /\textipa{d@U."meIn}/\\
\cellcolor{gray!6}{drown} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{draUn}/}\\
elude & /01/ & /\textipa{I."lu:d}/\\
\cellcolor{gray!6}{event} & \cellcolor{gray!6}{/01/} & \cellcolor{gray!6}{/\textipa{I."vent}/}\\
\addlinespace
eventually & /01000/ & /\textipa{I."ventS.u.@l.i}/\\
\cellcolor{gray!6}{figure} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"fIg.@}/}\\
fundamental & /2010/ & /\textipa{""f2nd.@."ment.@l}/\\
\cellcolor{gray!6}{furniture} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"f3:n.ItS.@}/}\\
general & /100/ & /\textipa{"dZen.@r.@l}/\\
\addlinespace
\cellcolor{gray!6}{generic} & \cellcolor{gray!6}{/010/} & \cellcolor{gray!6}{/\textipa{dZ@."ner.Ik}/}\\
hierarchy & /100/ & /\textipa{"haI@r.A:k.i}/\\
\cellcolor{gray!6}{identity} & \cellcolor{gray!6}{/0100/} & \cellcolor{gray!6}{/\textipa{aI."dent.@t.i}/}\\
lurid & /10/ & /\textipa{"ljU@r.Id}/\\
\cellcolor{gray!6}{manufacture} & \cellcolor{gray!6}{/2010/} & \cellcolor{gray!6}{/\textipa{""m\ae n.ju."f\ae k.tS@}/}\\
\addlinespace
mayor & /1/ & /\textipa{me@}/\\
\cellcolor{gray!6}{measure} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"meZ.@}/}\\
metaphor & /100/ & /\textipa{"met.@f.@}/\\
\cellcolor{gray!6}{natural} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"n\ae tS.@r.@l}/}\\
nuance & /10/ & /\textipa{"nju:.A:ns}/\\
\addlinespace
\cellcolor{gray!6}{professional} & \cellcolor{gray!6}{/0100/} & \cellcolor{gray!6}{/\textipa{pr@."feS.@n.@l}/}\\
purpose & /10/ & /\textipa{"p3:p.@s}/\\
\cellcolor{gray!6}{rhythm} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"rID.@m}/}\\
rough & /1/ & /\textipa{r2f}/\\
\cellcolor{gray!6}{table} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"teIb.@l}/}\\
\addlinespace
theme & /1/ & /\textipa{Ti:m}/\\
\cellcolor{gray!6}{thought} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{TO:t}/}\\
through & /1/ & /\textipa{Tru:}/\\
\cellcolor{gray!6}{treatise} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"tri:t.Iz}/}\\
virtue & /10/ & /\textipa{"v3:tS.u:}/\\
\addlinespace
\cellcolor{gray!6}{sheet} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{Si:t}/}\\
parable & /100/ & /\textipa{"p\ae r.@b.@l}/\\
\cellcolor{gray!6}{carnage} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"kA:n.IdZ}/}\\
widen & /10/ & /\textipa{"waId.@n}/\\
\cellcolor{gray!6}{pervasive} & \cellcolor{gray!6}{/010/} & \cellcolor{gray!6}{/\textipa{p@."veIs.Iv}/}\\
\addlinespace
cruel & /10/ & /\textipa{"kru:.@l}/\\
\cellcolor{gray!6}{sear} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{sI@}/}\\
criticism & /1020/ & /\textipa{"krIt.I.""sIz.@m}/\\
\cellcolor{gray!6}{aesthetic} & \cellcolor{gray!6}{/010/} & \cellcolor{gray!6}{/\textipa{i:s."Tet.Ik}/}\\
parallel & /100/ & /\textipa{"p\ae r.@.lel}/\\
\addlinespace
\cellcolor{gray!6}{bald} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{bO:ld}/}\\
thick & /1/ & /\textipa{TIk}/\\
\cellcolor{gray!6}{typical} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"tIp.Ik.@l}/}\\
endeavor & /010/ & /\textipa{In."dev.@}/\\
\cellcolor{gray!6}{hideous} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"hId.i.@s}/}\\
\addlinespace
border & /10/ & /\textipa{"bO:d.@}/\\
\cellcolor{gray!6}{outlaw} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"aUt.lO:}/}\\
slavery & /100/ & /\textipa{"sleIv.@r.i}/\\
\cellcolor{gray!6}{colony} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"k6l.@n.i}/}\\
kernel & /10/ & /\textipa{"k3:n.@l}/\\
\addlinespace
\cellcolor{gray!6}{nature} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"neItS.@}/}\\
rehearsal & /010/ & /\textipa{ri."h3:s.@l}/\\
\cellcolor{gray!6}{echo} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"ek.@U}/}\\
warehouse & /10/ & /\textipa{"we@.haUs}/\\
\cellcolor{gray!6}{proposal} & \cellcolor{gray!6}{/010/} & \cellcolor{gray!6}{/\textipa{pr@."p@Uz.@l}/}\\
\addlinespace
skew & /1/ & /\textipa{skju:}/\\
\cellcolor{gray!6}{scrutiny} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"skru:t.In.i}/}\\
lavish & /10/ & /\textipa{"l\ae v.IS}/\\
\cellcolor{gray!6}{enquiry} & \cellcolor{gray!6}{/0100/} & \cellcolor{gray!6}{/\textipa{In."kwaI.@r.i}/}\\
crowd & /1/ & /\textipa{kraUd}/\\
\addlinespace
\cellcolor{gray!6}{opinion} & \cellcolor{gray!6}{/010/} & \cellcolor{gray!6}{/\textipa{@."pIn.j@n}/}\\
critic & /10/ & /\textipa{"krIt.Ik}/\\
\cellcolor{gray!6}{marriage} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"m\ae r.IdZ}/}\\
volume & /10/ & /\textipa{"v6l.ju:m}/\\
\cellcolor{gray!6}{history} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"hIs.tri}/}\\
\addlinespace
appreciative & /01000/ & /\textipa{@."pri:S.i.@t.Iv}/\\
\cellcolor{gray!6}{plague} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{pleIg}/}\\
shelter & /10/ & /\textipa{"Selt.@}/\\
\cellcolor{gray!6}{pour} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{pO:}/}\\
parliament & /100/ & /\textipa{"pA:l.@.m@nt}/\\
\addlinespace
\cellcolor{gray!6}{structure} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"str2k.tS@}/}\\
medieval & /2010/ & /\textipa{""med.i."i:v.@l}/\\
\cellcolor{gray!6}{reason} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"ri:z.@n}/}\\
era & /10/ & /\textipa{"I@r.@}/\\
\cellcolor{gray!6}{factory} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"f\ae k.tri}/}\\
\addlinespace
architecture & /1000/ & /\textipa{"A:k.I.tek.tS@}/\\
\cellcolor{gray!6}{cheap} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{tSi:p}/}\\
falsehood & /10/ & /\textipa{"fO:ls.hUd}/\\
\cellcolor{gray!6}{fantasy} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"f\ae nt.@s.i}/}\\
hefty & /10/ & /\textipa{"heft.i}/\\
\addlinespace
\cellcolor{gray!6}{frame} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{freIm}/}\\
enthusiastic & /02010/ & /\textipa{In.""Tju:z.i."\ae st.Ik}/\\
\cellcolor{gray!6}{perceive} & \cellcolor{gray!6}{/01/} & \cellcolor{gray!6}{/\textipa{p@."si:v}/}\\
stance & /1/ & /\textipa{st\ae ns}/\\
\cellcolor{gray!6}{polish} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"p6l.IS}/}\\
\addlinespace
item & /10/ & /\textipa{"aIt.@m}/\\
\cellcolor{gray!6}{journey} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"dZ3:n.i}/}\\
common & /10/ & /\textipa{"k6m.@n}/\\
\cellcolor{gray!6}{process} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"pr@Us.es}/}\\
allow & /01/ & /\textipa{@."laU}/\\
\addlinespace
\cellcolor{gray!6}{choose} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{tSu:z}/}\\
average & /100/ & /\textipa{"\ae v.@r.IdZ}/\\
\cellcolor{gray!6}{study} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"st2d.i}/}\\
attach & /01/ & /\textipa{@."t\ae tS}/\\
\cellcolor{gray!6}{sequence} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"si:k.w@ns}/}\\
\addlinespace
template & /10/ & /\textipa{"tem.pleIt}/\\
\cellcolor{gray!6}{isolate} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"aIs.@.leIt}/}\\
approval & /010/ & /\textipa{@."pru:v.@l}/\\
\cellcolor{gray!6}{palate} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"p\ae l.@t}/}\\
cleanse & /1/ & /\textipa{klenz}/\\
\addlinespace
\cellcolor{gray!6}{focus} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"f@Uk.@s}/}\\
closure & /10/ & /\textipa{"kl@UZ.@}/\\
\cellcolor{gray!6}{analogous} & \cellcolor{gray!6}{/0100/} & \cellcolor{gray!6}{/\textipa{@."n\ae l.@g.@s}/}\\
habit & /10/ & /\textipa{"h\ae b.It}/\\
\cellcolor{gray!6}{widow} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"wId.@U}/}\\
\addlinespace
magazine & /201/ & /\textipa{""m\ae g.@."zi:n}/\\
\cellcolor{gray!6}{gradient} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"greId.i.@nt}/}\\
palette & /10/ & /\textipa{"p\ae l.@t}/\\
\cellcolor{gray!6}{height} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{haIt}/}\\
visual & /100/ & /\textipa{"vIZ.u.@l}/\\
\addlinespace
\cellcolor{gray!6}{alignment} & \cellcolor{gray!6}{/010/} & \cellcolor{gray!6}{/\textipa{@."laIn.m@nt}/}\\
smooth & /1/ & /\textipa{smu:D}/\\
\cellcolor{gray!6}{deadline} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"ded.laIn}/}\\
once & /1/ & /\textipa{w2ns}/\\
\cellcolor{gray!6}{early} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"3:l.i}/}\\
\addlinespace
receive & /01/ & /\textipa{ri."si:v}/\\
\cellcolor{gray!6}{schedule} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"Sed.ju:l}/}\\
lengthy & /10/ & /\textipa{"leNT.i}/\\
\cellcolor{gray!6}{signature} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"sIg.n@tS.@}/}\\
summary & /100/ & /\textipa{"s2m.@r.i}/\\
\addlinespace
\cellcolor{gray!6}{method} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"meT.@d}/}\\
yield & /1/ & /\textipa{ji:@ld}/\\
\cellcolor{gray!6}{squirrel} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"skwIr.@l}/}\\
result & /01/ & /\textipa{ri."z2lt}/\\
\cellcolor{gray!6}{outcome} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"aUt.k2m}/}\\
\addlinespace
trend & /1/ & /\textipa{trend}/\\
\cellcolor{gray!6}{approach} & \cellcolor{gray!6}{/01/} & \cellcolor{gray!6}{/\textipa{@."pr@UtS}/}\\
scheme & /1/ & /\textipa{ski:m}/\\
\cellcolor{gray!6}{level} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"lev.@l}/}\\
both & /1/ & /\textipa{b@UT}/\\
\addlinespace
\cellcolor{gray!6}{recipe} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"res.@p.i}/}\\
period & /100/ & /\textipa{"pI@r.i.@d}/\\
\cellcolor{gray!6}{area} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"e@r.i.@}/}\\
logo & /10/ & /\textipa{"l@Ug.@U}/\\
\cellcolor{gray!6}{within} & \cellcolor{gray!6}{/01/} & \cellcolor{gray!6}{/\textipa{wID."In}/}\\
\addlinespace
status & /10/ & /\textipa{"steIt.@s}/\\
\cellcolor{gray!6}{pudding} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"pUd.IN}/}\\
heart & /1/ & /\textipa{hA:t}/\\
\cellcolor{gray!6}{range} & \cellcolor{gray!6}{/1/} & \cellcolor{gray!6}{/\textipa{reIndZ}/}\\
afford & /01/ & /\textipa{@."fO:d}/\\
\addlinespace
\cellcolor{gray!6}{product} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"pr6d.2kt}/}\\
retailer & /100/ & /\textipa{"ri:.teI@l.@}/\\
\cellcolor{gray!6}{improve} & \cellcolor{gray!6}{/01/} & \cellcolor{gray!6}{/\textipa{Im."pru:v}/}\\
visionary & /1000/ & /\textipa{"vIZ.@n.@r.i}/\\
\cellcolor{gray!6}{different} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"dIf.r@nt}/}\\
\addlinespace
obituary & /01000/ & /\textipa{@."bItS.u.@r.i}/\\
\cellcolor{gray!6}{revival} & \cellcolor{gray!6}{/010/} & \cellcolor{gray!6}{/\textipa{ri."vaIv.@l}/}\\
flourish & /10/ & /\textipa{"fl2r.IS}/\\
\cellcolor{gray!6}{intricate} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"Intr.Ik.@t}/}\\
lively & /10/ & /\textipa{"laIv.li}/\\
\addlinespace
\cellcolor{gray!6}{pleasure} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"pleZ.@}/}\\
author & /10/ & /\textipa{"O:T.@}/\\
\cellcolor{gray!6}{entire} & \cellcolor{gray!6}{/010/} & \cellcolor{gray!6}{/\textipa{In."taI.@}/}\\
riot & /10/ & /\textipa{"raI.@t}/\\
\cellcolor{gray!6}{placard} & \cellcolor{gray!6}{/10/} & \cellcolor{gray!6}{/\textipa{"pl\ae k.A:d}/}\\
\addlinespace
casualty & /1000/ & /\textipa{"k\ae Z.u.@l.ti}/\\
\cellcolor{gray!6}{digital} & \cellcolor{gray!6}{/100/} & \cellcolor{gray!6}{/\textipa{"dIdZ.It.@l}/}\\
\bottomrule
\end{longtable}



##  Échéances 


\begin{longtable}{>{\raggedright\arraybackslash}p{3cm}>{\raggedright\arraybackslash}p{13cm}}
\toprule
Échéances & Instructions\\
\midrule
\cellcolor{gray!6}{24/09/2021} & \cellcolor{gray!6}{DN3Créa : Préparer la présentation orale des résultats de sondage}\\
28/09/2021 & DN3Ang2 : Préparer la présentation orale des résultats de sondage\\
\cellcolor{gray!6}{29/11/2021} & \cellcolor{gray!6}{Rendre l'abstract rédigé en anglais du mémoire}\\
\bottomrule
\end{longtable}



## Flashcards

NULL
